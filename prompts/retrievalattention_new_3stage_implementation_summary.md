...
## 12. Multi-GPU reproducibility fix notes (2026-04-09)

### 12.1 Observed issue
When running the 3-stage benchmark with multiple visible GPUs, results diverged from the single-GPU run:
- final-verify reject rate increased sharply,
- generated text drifted,
- and settlement behavior was inconsistent.

### 12.2 Root causes identified
1. **Backend cache-state sync gap for RetroInfer valid lengths**
- The backend snapshot/revert/sync path handled `valid_length_dict` (Full_Flash_Attn) but did not handle `valid_lengths_dict` (RetroInfer cache variants).
- This caused replay/revert state drift risk in multi-stage cache lifecycle operations.

2. **Benchmark device-policy mismatch under multiple visible GPUs**
- `tests/RetrievalAttention_new/selfspec_benchmark.py` previously auto-selected `model_device=auto` whenever `torch.cuda.device_count() > 1`.
- That switched execution into model-parallel path only in the multi-visible-GPU run, so the A/B comparison was not an apples-to-apples reproducibility test against the single-GPU path.

### 12.3 Code changes made

#### A) Backend hardening (`Engine/RetrievalAttention_new/backend.py`)
- `CacheSnapshot` extended with `valid_lengths_dict`.
- Added helpers to clone/restore both:
   - `valid_length_dict`
   - `valid_lengths_dict`
- Updated state-management paths to support both forms:
   - `snapshot_state`
   - `revert_to`
   - `truncate_to`
   - `_sync_committed_growth`
   - `cache_state_report`
- Added `cluster_size_cumsum` growth-sync copy in `_sync_committed_growth` when compatible.
- Added multi-GPU CUDA barriers in stage-cache lifecycle boundaries:
   - cache activation and decode-step boundaries,
   - snapshot/revert/commit boundaries.

#### B) Benchmark policy update (`tests/RetrievalAttention_new/selfspec_benchmark.py`)
- Added CLI arg:
   - `--model_device` (default: `cuda:0`)
- Device selection logic updated:
   - default behavior uses single-device model execution (`cuda:0`) for deterministic reproducibility,
   - `--model_device auto` is now explicit opt-in.
- Added runtime notice when multiple GPUs are visible but benchmark is intentionally using single-device mode.

### 12.4 Validation outcomes

#### Reduced repro (fast check)
Settings:
- `prefix_len=4096`, `num_max_token=32`, `gamma1=6`, `gamma2=16`, `budget1=0.02`, `budget2=0.20`, `seed=123`

Post-fix parity:
- `CUDA_VISIBLE_DEVICES=2` and `CUDA_VISIBLE_DEVICES=2,3` produced matching final-verify accepted/rejected pattern and matching generated continuation under default benchmark policy.

#### Full-size repro (test.sh-equivalent)
Settings:
- `prefix_len=32768`, `num_max_token=100`, `gamma1=6`, `gamma2=32`, `budget1=0.02`, `budget2=0.20`, `seed=123`

Post-fix parity:
- Single visible GPU and multi-visible GPU runs both produced:
   - `Speculate calls: 102`
   - `Early Verify calls: 17`
   - `Final Verify calls: 3`
   - `Tokens generated: 100`
   - Final verify stage accepted/rejected sequence: `34/0`, `37/0`, `29/0`
   - Matching final-verify text chunks for all three settlements.

### 12.5 Practical guidance
- For reproducibility comparisons across different `CUDA_VISIBLE_DEVICES` settings, keep default `--model_device cuda:0`.
- Use `--model_device auto` only when explicitly evaluating model-parallel behavior, and treat it as a separate mode rather than a strict reproducibility baseline against single-device runs.
