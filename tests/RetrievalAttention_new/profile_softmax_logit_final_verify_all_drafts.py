...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Profile draft-token softmax probabilities and final-verify rejection histogram (2-stage only)."
    )
    ...
    parser.add_argument(
        "--logs_dir",
        type=str,
        default="MagicDec/tests/RetrievalAttention_new/logs/softmax_logit_profile",
    )
...
def prob_to_bin_index(prob_value, bin_width, num_bins):
    clipped = min(1.0, max(0.0, float(prob_value)))
    raw = int(clipped / bin_width)
    return min(raw, num_bins - 1)


def build_histogram_rows(bin_width, profiled_drafted_counts, reject_rate_drafted_counts, rejected_counts):
    rows = []
    for idx, profiled_drafted_count in enumerate(profiled_drafted_counts):
        left = idx * bin_width
        right = min(1.0, (idx + 1) * bin_width)
        center = (left + right) / 2.0
        reject_rate_drafted_count = reject_rate_drafted_counts[idx]
        rejected_count = rejected_counts[idx]
        reject_rate = (
            (rejected_count / reject_rate_drafted_count) if reject_rate_drafted_count > 0 else 0.0
        )
        rows.append(
            {
                "bin_index": idx,
                ...
                "drafted_token_count": int(profiled_drafted_count),
                "reject_rate_drafted_token_count": int(reject_rate_drafted_count),
...
def write_histogram_csv(path, rows):
    headers = [
        ...
        "bin_center",
        "drafted_token_count",
        "reject_rate_drafted_token_count",
...
def softmax_probs_for_drafted_tokens(logits, drafted_tokens):
    batch_size, num_steps = drafted_tokens.shape

    # decode_forward logits may arrive as [1, steps, 1, vocab], [steps, 1, vocab],
    # [1, steps, vocab], or already normalized as [batch, steps, vocab].
    while logits.dim() > 3:
        squeeze_dim = None
        for dim in range(logits.dim() - 1):
            if logits.shape[dim] == 1:
                squeeze_dim = dim
                break
        if squeeze_dim is None:
            raise ValueError(
                f"Unexpected logits shape {tuple(logits.shape)} for drafted token shape {tuple(drafted_tokens.shape)}"
            )
        logits = logits.squeeze(squeeze_dim)

    if logits.dim() == 2:
        if logits.shape[0] != num_steps:
            raise ValueError(
                f"Unexpected 2D logits shape {tuple(logits.shape)} for drafted token shape {tuple(drafted_tokens.shape)}"
            )
        logits = logits.unsqueeze(0)
    elif logits.dim() == 3:
        if logits.shape[0] == batch_size and logits.shape[1] == num_steps:
            pass
        elif logits.shape[0] == num_steps and logits.shape[1] == batch_size:
            logits = logits.transpose(0, 1)
        else:
            raise ValueError(
                f"Unexpected 3D logits shape {tuple(logits.shape)} for drafted token shape {tuple(drafted_tokens.shape)}"
            )
    else:
        raise ValueError(
            f"Unexpected logits rank {logits.dim()} for drafted token shape {tuple(drafted_tokens.shape)}"
        )

    probs = torch.softmax(logits, dim=-1)
    token_indices = drafted_tokens.unsqueeze(-1).to(dtype=torch.long)
    return probs.gather(dim=-1, index=token_indices).squeeze(-1)
...
def main():
    ...
    reject_rate_drafted_counts = [0 for _ in range(num_bins)]
    rejected_counts = [0 for _ in range(num_bins)]

    total_profiled_drafted_tokens = 0
    ...
    for step in tqdm(range(total_steps), total=total_steps):
        ...
        while generated_tokens < args.num_max_token:
            ...
            drafted_tokens, _, drafted_logits = engine.speculate_with_features_and_logits(current_token, draft_steps)
            if drafted_tokens.numel() == 0:
                break

            drafted_token_probs = softmax_probs_for_drafted_tokens(drafted_logits, drafted_tokens)

            eos_idx = first_eos_idx(drafted_tokens, eos_id)
            if eos_idx >= 0:
                drafted_tokens = drafted_tokens[:, : eos_idx + 1]
                drafted_token_probs = drafted_token_probs[:, : eos_idx + 1]
            ...
            for idx in range(drafted_tokens.shape[1]):
                prob_value = float(drafted_token_probs[0, idx])
                bin_idx = prob_to_bin_index(prob_value, args.bin_width, num_bins)
                drafted_counts[bin_idx] += 1
                total_profiled_drafted_tokens += 1

                if idx >= included_len:
                    continue

                reject_rate_drafted_counts[bin_idx] += 1
    ...
    rows = build_histogram_rows(
        args.bin_width,
        drafted_counts,
        reject_rate_drafted_counts,
        rejected_counts,
    )
    ...
    print(f"total_profiled_drafted_tokens: {total_profiled_drafted_tokens}")
