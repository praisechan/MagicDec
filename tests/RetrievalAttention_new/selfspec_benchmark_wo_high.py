...
def parse_args():
    parser = argparse.ArgumentParser(description="RetrievalAttention_new 3-stage self-spec benchmark without high-mode cache")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Ignored in selfspec_benchmark_wo_high.py. Kept only for CLI compatibility.",
    )
...
def replay_verified_prefix(engine, draft_snapshot, early_snapshot, verify_start_token, committed_online):
    engine.revert_to("draft", draft_snapshot)
    engine.revert_to("early_verify", early_snapshot)

    accepted_prefix = committed_online[:, :-1]
    # Each cache must replay the committed prefix under its own budget so later
    # mode switches do not inherit another verifier's approximation trajectory.
    ...
    for step in tqdm(range(total_steps), total=total_steps):
        ...
        while len(emitted_tokens) < args.num_max_token:
            ...
            if dynamic_mode == "skip":
                ...
                # Normal after skips: verify the full stacked span in one pass.
            ...
            # In normal mode we materialize immediately.
