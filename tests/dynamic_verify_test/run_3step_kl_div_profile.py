...
parser.add_argument("--topk", type=int, default=100, help="number of top tokens to consider for KL divergence")
...
if args.dataset == "pg19":
  num_eval_steps = min(30, len(dataloader))
...
profile_dir = f"/home/juchanlee/MagicDec/profile/data_kl_profile_cluster32_gamma{args.gamma2}/{MODEL}_{args.dataset}_{args.prefix_len}"
...
for step, batch in tqdm(enumerate(dataset), total=num_eval_steps):
    ...
    while not terminal:
        ...
        kl_confidence_analyzer.store_draft_data(draft_logits, top1_top2_diff, use_extended_verification=False)
        ...
        print(f"Verify analysis: {num_accepted} tokens accepted, draft_conf_len={len(kl_confidence_analyzer.current_draft_confidences) if kl_confidence_analyzer.current_draft_confidences else 0}, verify_conf_len={len(kl_confidence_analyzer.current_verify_confidences) if kl_confidence_analyzer.current_verify_confidences else 0}")
                
        # Accumulate confidences for later settle analysis
        # Store verify data for KL analysis
        kl_confidence_analyzer.store_verify_data(verify_outputs[0], verify_logits, verify_top1_top2_diff, num_accepted, use_extended_verification=False)
        print(f"Accumulated confidences now: {len(kl_confidence_analyzer.accumulated_draft_confidences)} tokens")
        ...
        if num_unsettled_tokens >= args.gamma2 or called_verify > 2 * (args.gamma2 / args.gamma1) or terminal:
            ...
            print(f"Settle analysis: {settle_accepted} tokens accepted out of {len(kl_confidence_analyzer.accumulated_draft_confidences)} accumulated tokens")
    ...
    # Store kl confidence analyzer's rejected tokens data to CSV
    kl_confidence_analyzer.raise_step_idx()
...
kl_confidence_analyzer.save_histograms(f"confidence_analysis_topk{args.topk}", filename_prefix)
