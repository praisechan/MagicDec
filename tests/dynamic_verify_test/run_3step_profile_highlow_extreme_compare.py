...
from confidence_analyzer_old import ConfidenceAnalyzer
...
parser.add_argument("--trace_final_verify_tokens", type=int, default=32, help="number of settle-stage tokens to trace")
...
def flatten_token_ids(token_like):
    if isinstance(token_like, torch.Tensor):
        return token_like.detach().cpu().reshape(-1).tolist()
    return torch.as_tensor(token_like).reshape(-1).tolist()


def decode_token_ids(tokenizer, token_ids, max_chars=400):
    if len(token_ids) == 0:
        return ""
...
for step, batch in tqdm(enumerate(dataset), total=num_eval_steps):
    ...
    prefix_ids = flatten_token_ids(input_ids[0])
    print(f"[COMPARE] Step {step} prefix_len={len(prefix_ids)}")
    print(f"[COMPARE] Step {step} prefix_ids_head={prefix_ids[:64]}")
    print(f"[COMPARE] Step {step} prefix_text_head={decode_token_ids(tokenizer, prefix_ids[:256])}")
    ...
    called_verify = 0
    while not terminal:
        current_generated_tokens = to_int_scalar(engine.settled_cachelength - input_len)
        if current_generated_tokens >= num_gen_token_max:
            print(f"[STOP] Reached {current_generated_tokens} generated tokens (limit={num_gen_token_max}).")
            break
        ...
        draft_ids = flatten_token_ids(draft_outputs)
        stage_outputs_data.append({
            "stage": "draft",
            "outputs": draft_ids
        })
        print(f"[COMPARE] draft_ids={draft_ids}")
        print(f"[COMPARE] draft_text={decode_token_ids(tokenizer, draft_ids)}")
        ...
        if pass_verify:
            ...
            verify_ids = flatten_token_ids(verify_outputs)
            stage_outputs_data.append({
                "stage": "verify",
                "outputs": verify_ids
            })
            print(f"[COMPARE] verify_ids={verify_ids}")
            print(f"[COMPARE] verify_text={decode_token_ids(tokenizer, verify_ids)}")
        ...
        if num_unsettled_tokens >= args.gamma2 or called_verify > 2 * (args.gamma2 / args.gamma1) or terminal:
            ...
            settle_ids = flatten_token_ids(settle_outputs)
            stage_outputs_data.append({
                "stage": "settle",
                "outputs": settle_ids
            })
            print(f"[COMPARE] settle_ids={settle_ids}")
            print(f"[COMPARE] settle_text={decode_token_ids(tokenizer, settle_ids)}")

            if len(final_verify_trace_tokens) < args.trace_final_verify_tokens:
                remaining = args.trace_final_verify_tokens - len(final_verify_trace_tokens)
                final_verify_trace_tokens.extend(settle_ids[:remaining])
                print(
                    f"[COMPARE] final_verify_trace_progress={len(final_verify_trace_tokens)}/{args.trace_final_verify_tokens}, "
                    f"latest={final_verify_trace_tokens}"
                )
            ...
            if current_generated_tokens >= num_gen_token_max:
                if current_generated_tokens > num_gen_token_max:
                    overflow = current_generated_tokens - num_gen_token_max
                    engine.settled_cachelength = engine.settled_cachelength - overflow
                    num_nodes = num_nodes - overflow
                    print(f"[STOP] Truncated overflow by {overflow} tokens to enforce limit={num_gen_token_max}.")
                    current_generated_tokens = to_int_scalar(engine.settled_cachelength - input_len)
                print(f"[STOP] Reached {current_generated_tokens} generated tokens (limit={num_gen_token_max}).")
                terminal = True
    ...
    final_generated_tokens = to_int_scalar(engine.settled_cachelength - input_len)
    if final_generated_tokens > num_gen_token_max:
        overflow = final_generated_tokens - num_gen_token_max
        engine.settled_cachelength = engine.settled_cachelength - overflow
        num_nodes = num_nodes - overflow
        final_generated_tokens = to_int_scalar(engine.settled_cachelength - input_len)
        print(f"[STOP] Final clamp removed {overflow} overflow tokens; generated={final_generated_tokens}.")
...
print(f"[COMPARE] final_verify_trace_tokens({len(final_verify_trace_tokens)}): {final_verify_trace_tokens}")
print(f"[COMPARE] final_verify_trace_text: {decode_token_ids(tokenizer, final_verify_trace_tokens, max_chars=1000)}")
