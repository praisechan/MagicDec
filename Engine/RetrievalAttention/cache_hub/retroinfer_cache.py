...
class retroinfer_cache(KV_Cache):
    ...
    def __init__(
        ...
        if "SUPERCLUSTER_SIZE" in os.environ:
            self.approx_supercluster_size = int(os.environ["SUPERCLUSTER_SIZE"])
        else:
            self.approx_supercluster_size = 4 # default        
        self.n_super_centroids = int(n_centroids/self.approx_supercluster_size)
        self.profile_clustering = profile_clustering
        self.profile_super_clustering = False
        ...
        self.selection_ratio = self.nprobe /self.n_centroids
        ...
        # if self.profile_super_clustering:
    ...
    def compute(self, queries, layer_idx):
        ...
        if self.store_decoding_data:
            self.store_decoding_data = False # this works like mutex in OS
            torch.save(selected_cI, f"{self.outdir_path}/selected_cI_step{self.decoding_step}_layer{layer_idx}.pt")
        ...
        if self.profile_super_clustering:
            ...
            # calculate zero supercluster ratio
            zero_ratios = ratios[ratios == 0]  # Filter out zero values
            non_selected_sc_ratio = len(zero_ratios)/len(ratios)
            CSV_PATH = f'/home/juchanlee/MagicDec/output/supercluster_indexing/non_selected_supercluster_ratio.csv'
            # if the file doesn't yet exist, write the header
            import csv
            if not os.path.exists(CSV_PATH):
                with open(CSV_PATH, "w", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow(["seq_len", "budget_ratio", "supercluster_size", "layer_idx", "zero_ratio"])
            ...
            with open(CSV_PATH, "a", newline="") as f:
                ...
                writer.writerow([
                  ...
                  self.approx_supercluster_size,
                  layer_idx,
                  non_selected_sc_ratio
                ])      
            ...
            # output_path = f'/home/juchanlee/MagicDec/supercluster_indexing/selected_cluster_ratio_seq{self.seq_len}_budget{self.selection_ratio:.2f}_scsize{self.approx_supercluster_size}.png'
